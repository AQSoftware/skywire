...
var playerData = { distance: 0, score: 0, targetScore: "" };
 ...
 * 
 */
function buildGameButton(_oData) {
	//console.log(_oData)
	if (_oData.targetScore) playerData.targetScore = _oData.targetScore;
...
///			if (playerData.targetScore != "") txtReach.text = "Reach " + playerData.targetScore + "KM";
		...
		case 'result':
			//console.log(AQCore.Utils.relativeToAbsolutePath(_props.data.engagementInfo.backgroundBig), Number((playerData.distance * .0005).toFixed(1)) + 3)
			...
			toggleTransition(resultContainer, true);
			stopGame();
...
function toggleTransition(obj, con) {
	if (con) {
		...
		TweenMax.to(obj, .5, {
			y: -800, overwrite: true, onComplete: function () {
				obj.visible = false;
			}
		});
	}
...
function blurStage() {
	...
	stage.cache(0, 0, $(window).width() * 2, $(window).height() * 2);
...
function movePlayerPosX() {
	//console.log((canvasW / 2) / 100 * 120)
	var widthPercentage = (canvasW / 2) / 100 * 120;
...
function updateDistance() {
	...
	//playerData.targetScore = distanceDisplay.replace('[NUMBER]', playerData.targetScore)
	//playerData.targetscore = playerData.targetscore.replace('[NUMBER]', playerData.targetscore)

	if (playerData.targetScore != "") {
		txtDistance.text = playerData.score + "/" + playerData.targetScore + "KM";
		txtDistanceResult.text = playerData.score + "/" + playerData.targetScore + "KM";
	} else {
...
function updateBuildings() {
	...
	for (n = 0; n < buildingsLoop_arr.length; n++) {
		...
		if ($.building[buildingsLoop_arr[n]].y > (canvasH + 400) * 2) {
			removeBuilding(n);
		}
	}
}
